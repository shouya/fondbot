...
impl Bot {
  ...
  pub fn send_raw<T: Into<String>>(&self,
    ...
    let txt = txt.into();
    while let Err(err) =
      self.api.send_message(chat_id,                // chat id
                            txt.clone(),            // txt
...
pub fn is_cmds(msg: &tg::Message, prefixes: &str) -> bool {
  for prefix in prefixes.split_whitespace() {
    if is_cmd(msg, prefix) { return true }
  }
  false
}
...
pub fn cmd_cmd(msg: &tg::Message) -> Option<String> {
  if let Some(txt) = msg_txt(msg) {
    if txt.len() <= 1                      { return None }
    if txt.chars().nth(0).unwrap() != '/'  { return None }
...
pub fn eat_updates(bot: &Bot) -> usize {
  let mut count = 0;
  let mut last = 0;
  while let Ok(updates) = bot.api.get_updates(Some(last), None, None) {
