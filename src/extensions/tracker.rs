...
impl BotExtension for Tracker {
  ...
  fn save(&self) -> JsonValue { JsonValue::Null /* TODO */ }
  fn load(&mut self, _: JsonValue) { /* TODO */ }
}

const BASE_URL: &'static str = "https://www.kuaidi100.com";
const USER_AGENT: &'static str = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.98 Safari/537.36";
...
impl Tracker {
  ...
  fn list(&mut self) -> String {
    ...
    for (k,v) in &self.trackers {
      let state = if Self::is_alive(v) { "[alive]" } else { "[dead ]" };
      out.push_str(&format!("`{}`\t{}\n", k, state));
    }
    out
  }
  ...
  fn request<URL: IntoUrl, T: Deserialize>(url: URL) -> Result<T> {
    ...
    let resp = try_strerr!(Client::new().get(url.clone())
  ...
  fn query_express_provider(tracking_no: &String) -> Result<String> {
    ...
    let url = format!("{}/autonumber/autoComNum?text={}",
                      BASE_URL, tracking_no);
...
impl ProgressTracker {
  ...
  fn update_progress(&self) {
    ...
    match query(&progress.nu, &progress.com) {
      Ok(new)  => { *progress = new; },
      Err(err) => { warn!("failed fetching progress: {:?}", err); }
    }
  }
