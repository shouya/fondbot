...
impl Afk {
  ...
  fn report_afk(&mut self, msg: &tg::Message, bot: &Bot) {
    if !self.is_afk()               { println!("not afk now"); return; }
    if !self.notification_expired() { println!("notify not expired"); return; }
  ...
  fn format_duration(d: &Duration) -> String {
    ...
    if d.num_days()    >= 1 { write!(&mut str, "{} days",  d.num_days())   ;}
    if d.num_hours()   >= 1 { write!(&mut str, "{} hours", d.num_hours())  ;}
    if d.num_minutes() >= 1 { write!(&mut str, "{} mins",  d.num_minutes());}
    if d.num_seconds() >= 1 { write!(&mut str, "{} secs",  d.num_seconds());}
    str
  }
